 ...
 */
function showWeatherCallback_en(data) {
    var channel = data.query.results.weather.rss.channel,
		...
		html = '<img style="vertical-align:bottom" height="52" width="52" src="' + img + '" /> <span style="font-size:44px">' + channel.item.condition.temp + '&deg;C</span><br />' +
				'<strong>Humidity    :</strong> ' + channel.atmosphere.humidity + '%<br />' +
				'<strong>Wind speed  :</strong> ' + channel.wind.speed + ' km/h<br />';
    document.write(html);
}
 ...
 */
function showWeather_en(location) {
    var baseUrl = 'http://query.yahooapis.com/v1/public/yql?q=',
		...
		url = baseUrl + encodeURIComponent(q) + '&format=json&diagnostics=false&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys&callback=showWeatherCallback_en';
